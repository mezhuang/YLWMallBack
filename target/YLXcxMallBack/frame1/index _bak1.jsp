...
//通过该方法与后台交互，确保推送时能找到指定用户  
 ...
 }else if(dwidth==1366 || dwidth==1360){
	 count=9;
	 if(<%=root_menu.size()<=9%>){
		 buttonable=false
	 }
 ...
 stepcarousel.setup({
	 ...
	 autostep: {enable:buttonable, moveby:count, pause:10000},
	 defaultbuttons: {enable: buttonable, moveby: count, leftnav: ['<%=basePath%>frame1/images/firstleft-2.png', -10, 25], rightnav: ['<%=basePath%>frame1/images/firstright-2.png', 5, 25]},
...
<script type="text/javascript">
	...
	function CloseTan(){
	 ...
	 if(dwidth==1280){
		 document.getElementById("mygallery").style.width="680px";
	 }else if(dwidth==1366 || dwidth==1360){
		 document.getElementById("mygallery").style.width="760px";
	 }else if(dwidth==1440){
		 document.getElementById("mygallery").style.width="840px";
	 }else if(dwidth==1920){
		 document.getElementById("mygallery").style.width="1240px";
	 }
	 $(function(){  
		 if(buttonable){
			 $('#rightbtn').click();

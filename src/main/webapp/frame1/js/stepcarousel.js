...
var stepcarousel={
	...
	addnavbuttons:function(config, currentpanel){
		config.$leftnavbutton=$('<img id="leftbtn" class="scrollbut" src="'+config.defaultbuttons.leftnav[0]+'">').css({zIndex:50, position:'absolute', left:config.offsets.left+config.defaultbuttons.leftnav[1]+'px', top:config.offsets.top+config.defaultbuttons.leftnav[2]+'px', cursor:'hand', cursor:'pointer'}).attr({title:'向左移动菜单'}).appendTo('body')
		config.$rightnavbutton=$('<img id="rightbtn" class="scrollbut" src="'+config.defaultbuttons.rightnav[0]+'">').css({zIndex:50, position:'absolute', left:config.offsets.left+config.$gallery.get(0).offsetWidth+config.defaultbuttons.rightnav[1]+'px', top:config.offsets.top+config.defaultbuttons.rightnav[2]+'px', cursor:'hand', cursor:'pointer'}).attr({title:'向右移动菜单'}).appendTo('body')
	...
	alignpanels:function($, config){
		...
		config.currentpanel=(config.panelbehavior.persist)? 0 : 0
